...
package body Radatracer.Rays.Tests is
   ...
   procedure Test_Ray_Calculations (T : in out AUnit.Test_Cases.Test_Case'Class) is
      R1 : constant Ray := (Origin => Make_Point (2, 3, 4), Direction => Make_Vector (1, 0, 0));
      R2 : constant Ray := (Origin => Make_Point (0, 0, -5), Direction => Make_Vector (0, 0, 1));
   begin
      AUnit.Assertions.Assert (Position (R1, 0.0) = Make_Point (2, 3, 4), "Ray position test 1");
      AUnit.Assertions.Assert (Position (R1, 1.0) = Make_Point (3, 3, 4), "Ray position test 2");
      AUnit.Assertions.Assert (Position (R1, -1.0) = Make_Point (1, 3, 4), "Ray position test 3");
